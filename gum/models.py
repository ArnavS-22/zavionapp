...
class Suggestion(Base):
    """Represents an AI-generated suggestion based on user behavior patterns.
    
    This model stores intelligent suggestions generated by the Gumbo engine when
    high-confidence propositions are detected, providing actionable insights to users.

    Attributes:
        id (int): Primary key for the suggestion.
        title (str): Short, descriptive title for the suggestion.
        description (str): Detailed description of the suggestion.
        category (str): Category/type of suggestion (e.g., 'productivity', 'workflow').
        rationale (str): AI reasoning behind the suggestion.
        expected_utility (float): Expected utility score (0.0-1.0).
        probability_useful (float): Probability the suggestion will be useful (0.0-1.0).
        trigger_proposition_id (int): ID of the proposition that triggered this suggestion.
        batch_id (str): Identifier for the batch this suggestion was generated in.
        delivered (bool): Whether the suggestion has been delivered to the frontend.
    ...
    trigger_proposition_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("propositions.id", ondelete="SET NULL"), nullable=True
    )
    ...
    def __repr__(self) -> str:
        ...
        return f"<Suggestion(id={self.id}, title={preview})>"
